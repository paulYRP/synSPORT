"""
Prepare TabSyn training datasets and model utility helpers.
...
class TabularDataset(Dataset):
    def __init__(self, X_num, X_cat):
        """
        Description:
            Initialize `TabularDataset` with the parameters required by later methods.
    ...
    def __getitem__(self, index):
        """
        Description:
            Return one indexed item from `TabularDataset`.
    ...
    def __len__(self):
        """
        Description:
            Return the number of items exposed by `TabularDataset`.
...
def preprocess(dataset_path, task_type = 'binclass', inverse = False, cat_encoding = None, concat = True):
    ...
    """
    Description:
        Preprocess data for `preprocess`.

    Input:
        dataset_path; task_type; inverse; cat_encoding; concat.
...
def update_ema(target_params, source_params, rate=0.999):
    """
    Description:
        Update values for `update_ema`.

    Input:
        target_params; source_params; rate.
...
def concat_y_to_X(X, y):
    """
    Description:
        Concatenate data for `concat_y_to_X`.
...
def make_dataset(
    ...
    """
    Description:
        Create objects for `make_dataset`.

    Input:
        data_path: str; T: src.Transformations; task_type; change_val: bool; concat.
