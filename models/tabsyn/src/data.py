"""
Load, transform, encode, and batch tabular datasets for TabSyn models.
...
class StandardScaler1d(StandardScaler):
    def partial_fit(self, X, *args, **kwargs):
        """
        Description:
            Partially fit data for `partial_fit`.
    ...
    def inverse_transform(self, X, *args, **kwargs):
        """
        Description:
            Run processing for `inverse_transform`.
...
def get_category_sizes(X: Union[torch.Tensor, np.ndarray]) -> List[int]:
    """
    Description:
        Return values for `get_category_sizes`.
...
class Dataset:
    ...
    def is_binclass(self) -> bool:
        """
        Description:
            Check whether data satisfies `is_binclass`.
    ...
    def is_multiclass(self) -> bool:
        """
        Description:
            Check whether data satisfies `is_multiclass`.
    ...
    def is_regression(self) -> bool:
        """
        Description:
            Check whether data satisfies `is_regression`.
    ...
    def n_num_features(self) -> int:
        """
        Description:
            Run processing for `n_num_features`.
    ...
    def n_cat_features(self) -> int:
        """
        Description:
            Run processing for `n_cat_features`.
    ...
    def nn_output_dim(self) -> int:
        """
        Description:
            Run processing for `nn_output_dim`.
    ...
    def calculate_metrics(
        ...
        """
        Description:
            Calculate values for `calculate_metrics`.

        Input:
            predictions: Dict[str, np.ndarray]; prediction_type: Optional[str].
...
def change_val(dataset: Dataset, val_size: float = 0.2):
    ...
    """
    Description:
        Change dataset values for `change_val`.

    Input:
        dataset: Dataset; val_size: float.
...
def num_process_nans(dataset: Dataset, policy: Optional[NumNanPolicy]) -> Dataset:
    ...
    """
    Description:
        Process numerical data for `num_process_nans`.

    Input:
        dataset: Dataset; policy: Optional[NumNanPolicy].
...
def normalize(
    ...
    """
    Description:
        Normalize data for `normalize`.

    Input:
        X: ArrayDict; normalization: Normalization; seed: Optional[int]; return_normalizer: bool.
...
def cat_process_nans(X: ArrayDict, policy: Optional[CatNanPolicy]) -> ArrayDict:
    """
    Description:
        Process categorical data for `cat_process_nans`.

    Input:
        X: ArrayDict; policy: Optional[CatNanPolicy].
...
def cat_drop_rare(X: ArrayDict, min_frequency: float) -> ArrayDict:
    """
    Description:
        Process categorical data for `cat_drop_rare`.

    Input:
        X: ArrayDict; min_frequency: float.
...
def cat_encode(
    ...
    """
    Description:
        Process categorical data for `cat_encode`.

    Input:
        X: ArrayDict; encoding: Optional[CatEncoding]; y_train: Optional[np.ndarray]; seed: Optional[int]; return_encoder: bool.

    Output:
        Tuple[ArrayDict, bool, Optional[Any]].
    """
    if encoding != 'counter':
        y_train = None
...
def transform_dataset(
    ...
    """
    Description:
        Transform data for `transform_dataset`.

    Input:
        dataset: Dataset; transformations: Transformations; cache_dir: Optional[Path]; return_transforms: bool.
...
def prepare_tensors(
    ...
    """
    Description:
        Prepare data for `prepare_tensors`.
...
class TabDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        """
        Description:
            Initialize `TabDataset` with the parameters required by later methods.

        Input:
            dataset: Dataset; split: Literal['train', 'val', 'test'].
    ...
    def __len__(self):
        """
        Description:
            Return the number of items exposed by `TabDataset`.
    ...
    def __getitem__(self, idx):
        """
        Description:
            Return one indexed item from `TabDataset`.
...
def prepare_dataloader(
    ...
    """
    Description:
        Prepare data for `prepare_dataloader`.

    Input:
        dataset: Dataset; split: str; batch_size: int.
...
def prepare_torch_dataloader(
    ...
    """
    Description:
        Prepare data for `prepare_torch_dataloader`.

    Input:
        dataset: Dataset; split: str; shuffle: bool; batch_size: int.
...
def dataset_from_csv(paths : Dict[str, str], cat_features, target, T):
    """
    Description:
        Run processing for `dataset_from_csv`.

    Input:
        paths: Dict[str, str]; cat_features; target; T.
...
class FastTensorDataLoader:
    ...
    def __init__(self, *tensors, batch_size=32, shuffle=False):
        """
        Description:
            Initialize `FastTensorDataLoader` with the parameters required by later methods.

        Input:
            batch_size; shuffle; *tensors.
    ...
    def __iter__(self):
        """
        Description:
            Initialize iteration over `FastTensorDataLoader`.
    ...
    def __next__(self):
        """
        Description:
            Return the next batch from `FastTensorDataLoader`.
    ...
    def __len__(self):
        """
        Description:
            Return the number of items exposed by `FastTensorDataLoader`.
...
def prepare_fast_dataloader(
    ...
    """
    Description:
        Prepare data for `prepare_fast_dataloader`.
...
def prepare_fast_torch_dataloader(
    ...
    """
    Description:
        Prepare data for `prepare_fast_torch_dataloader`.
...
def round_columns(X_real, X_synth, columns):
    """
    Description:
        Round values for `round_columns`.

    Input:
        X_real; X_synth; columns.
...
def concat_features(D : Dataset):
    """
    Description:
        Concatenate data for `concat_features`.
...
def concat_to_pd(X_num, X_cat, y):
    """
    Description:
        Concatenate data for `concat_to_pd`.
...
def read_changed_val(path, val_size=0.2):
    """
    Description:
        Read data for `read_changed_val`.
...
def load_dataset_info(dataset_dir_name: str) -> Dict[str, Any]:
    """
    Description:
        Load data for `load_dataset_info`.
