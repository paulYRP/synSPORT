...
def sample(net, num_samples, dim, num_steps = 50, device = 'cpu'):
    """
    Description:
        Sample synthetic data for `sample`.

    Input:
        net; num_samples; dim; num_steps; device.

    Output:
        Computed value returned by the function.
    """
    num_steps = max(2, int(num_steps or 50))
...
def sample_step(net, num_steps, i, t_cur, t_next, x_next):
    ...
    """
    Description:
        Sample synthetic data for `sample_step`.
...
class VPLoss:
    def __init__(self, beta_d=19.9, beta_min=0.1, epsilon_t=1e-5):
        """
        Description:
            Initialize `VPLoss` with the parameters required by later methods.

        Input:
            beta_d; beta_min; epsilon_t.
    ...
    def __call__(self, denosie_fn, data, labels, augment_pipe=None):
        """
        Description:
            Evaluate `VPLoss` as a callable loss or transform object.

        Input:
            denosie_fn; data; labels; augment_pipe.
...
class VELoss:
    def __init__(self, sigma_min=0.02, sigma_max=100, D=128, N=3072, opts=None):
        """
        Description:
            Initialize `VELoss` with the parameters required by later methods.

        Input:
            sigma_min; sigma_max; D; N; opts.
    ...
    def __call__(self, denosie_fn, data, labels = None, augment_pipe=None, stf=False, pfgmpp=False, ref_data=None):
        """
        Description:
            Evaluate `VELoss` as a callable loss or transform object.

        Input:
            denosie_fn; data; labels; augment_pipe; stf; pfgmpp; ref_data.
...
class EDMLoss:
    def __init__(self, P_mean=-1.2, P_std=1.2, sigma_data=0.5, hid_dim = 100, gamma=5, opts=None):
        """
        Description:
            Initialize `EDMLoss` with the parameters required by later methods.

        Input:
            P_mean; P_std; sigma_data; hid_dim; gamma; opts.
    ...
    def __call__(self, denoise_fn, data):
        ...
        """
        Description:
            Evaluate `EDMLoss` as a callable loss or transform object.
