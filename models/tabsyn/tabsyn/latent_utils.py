"""
Prepare and recover latent representations for TabSyn training and sampling.
...
def get_input_train(args):
    """
    Description:
        Return values for `get_input_train`.
...
def get_input_generate(args):
    """
    Description:
        Return values for `get_input_generate`.
...
def split_num_cat_target(syn_data, info, num_inverse, cat_inverse, device):
    """
    Description:
        Split data for `split_num_cat_target`.
...
def recover_data(syn_num, syn_cat, syn_target, info):
    ...
    """
    Description:
        Recover original-format data for `recover_data`.

    Input:
        syn_num; syn_cat; syn_target; info.
...
def process_invalid_id(syn_cat, min_cat, max_cat):
    """
    Description:
        Process data for `process_invalid_id`.

    Input:
        syn_cat; min_cat; max_cat.
