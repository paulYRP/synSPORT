"""
Define TabSyn VAE tokenizers, transformer blocks, encoder, decoder, and wrappers.
...
class Tokenizer(nn.Module):
    ...
    def __init__(self, d_numerical, categories, d_token, bias):
        """
        Description:
            Initialize `Tokenizer` with the parameters required by later methods.

        Input:
            d_numerical; categories; d_token; bias.
    ...
    def forward(self, x_num, x_cat):
        """
        Description:
            Run the forward pass for `Tokenizer`.
...
class MLP(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, dropout=0.5):
        """
        Description:
            Initialize `MLP` with the parameters required by later methods.
...
class MultiheadAttention(nn.Module):
    def __init__(self, d, n_heads, dropout, initialization = 'kaiming'):
        ...
        """
        Description:
            Initialize `MultiheadAttention` with the parameters required by later methods.

        Input:
            d; n_heads; dropout; initialization.
    ...
    def _reshape(self, x):
        """
        Description:
            Run the internal `_reshape` helper for `MultiheadAttention`.
    ...
    def forward(self, x_q, x_kv, key_compression = None, value_compression = None):
        ...
        """
        Description:
            Run the forward pass for `MultiheadAttention`.

        Input:
            x_q; x_kv; key_compression; value_compression.
...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        """
        Description:
            Initialize `Transformer` with the parameters required by later methods.

        Input:
            n_layers: int; d_token: int; n_heads: int; d_out: int; d_ffn_factor: int; attention_dropout; ffn_dropout; residual_dropout; activation; prenormalization; initialization.
        ...
        def make_normalization():
            """
            Description:
                Create objects for `make_normalization`.
    ...
    def _start_residual(self, x, layer, norm_idx):
        """
        Description:
            Run the internal `_start_residual` helper for `Transformer`.
    ...
    def _end_residual(self, x, x_residual, layer, norm_idx):
        """
        Description:
            Run the internal `_end_residual` helper for `Transformer`.

        Input:
            x; x_residual; layer; norm_idx.
    ...
    def forward(self, x):
        """
        Description:
            Run the forward pass for `Transformer`.
...
class AE(nn.Module):
    def __init__(self, hid_dim, n_head):
        """
        Description:
            Initialize `AE` with the parameters required by later methods.
    ...
    def get_embedding(self, x):
        """
        Description:
            Return values for `get_embedding`.
...
class VAE(nn.Module):
    def __init__(self, d_numerical, categories, num_layers, hid_dim, n_head = 1, factor = 4, bias = True):
        """
        Description:
            Initialize `VAE` with the parameters required by later methods.

        Input:
            d_numerical; categories; num_layers; hid_dim; n_head; factor; bias.
    ...
    def reparameterize(self, mu, logvar):
        """
        Description:
            Run processing for `reparameterize`.
...
class Reconstructor(nn.Module):
    def __init__(self, d_numerical, categories, d_token):
        """
        Description:
            Initialize `Reconstructor` with the parameters required by later methods.
    ...
    def forward(self, h):
        """
        Description:
            Run the forward pass for `Reconstructor`.
...
class Model_VAE(nn.Module):
    def __init__(self, num_layers, d_numerical, categories, d_token, n_head = 1, factor = 4,  bias = True):
        """
        Description:
            Initialize `Model_VAE` with the parameters required by later methods.

        Input:
            num_layers; d_numerical; categories; d_token; n_head; factor; bias.
    ...
    def forward(self, x_num, x_cat):
        ...
        """
        Description:
            Run the forward pass for `Model_VAE`.
...
class Encoder_model(nn.Module):
    def __init__(self, num_layers, d_numerical, categories, d_token, n_head, factor, bias = True):
        """
        Description:
            Initialize `Encoder_model` with the parameters required by later methods.
    ...
    def forward(self, x_num, x_cat):
        """
        Description:
            Run the forward pass for `Encoder_model`.
...
class Decoder_model(nn.Module):
    def __init__(self, num_layers, d_numerical, categories, d_token, n_head, factor, bias = True):
        """
        Description:
            Initialize `Decoder_model` with the parameters required by later methods.
    ...
    def forward(self, z):
        ...
        """
        Description:
            Run the forward pass for `Decoder_model`.
